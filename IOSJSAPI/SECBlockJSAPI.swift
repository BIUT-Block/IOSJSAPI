...
class SECBlockJSAPI: NSObject {
    ...
    /**
     ...
     - value: The signed tx
     */
    func txSign(_ tx: String, completion: @escaping (_ value: String) -> Void) {
    ...
    func privKeyToMnemonic(_ privKey: String, completion: @escaping (_ value: String) -> Void) {
        // Run this asynchronously in the background
        DispatchQueue.global(qos: .userInitiated).async {
            var Mnemonic = ""
            let jsModule = self.context.objectForKeyedSubscript("SECSDK")
    ...
    func mnemonicToPrivKey(_ mnemonic: String, completion: @escaping (_ value: String) -> Void) {
        // Run this asynchronously in the background
        DispatchQueue.global(qos: .userInitiated).async {
            var privKey = ""
            let jsModule = self.context.objectForKeyedSubscript("SECSDK")
